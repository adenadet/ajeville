<?php

namespace App\Models\Hrms;

use App\Models\Structure;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;

class Salary extends Structure
{
    use HasFactory;

    const StatusUnconfirmed = 1;
    const StatusConfirmed = 10;
    const StatusRejected = 40;

    protected $primaryKey = 'id';
    protected $table = 'hrms_salaries';
    protected $fillable = array('employee_id', 'period', 'net_pay', 'status', 'created_by', 'updated_by', 'deleted_by', 'created_at', 'updated_at', 'deleted_at');

    public function components(){
        return $this->hasMany('App\Models\Hrms\SalaryStructureComponent', 'salary_structure_id', 'id');
    }
    
    public function creator(){
        return $this->belongsTo('App\Models\User', 'created_by', 'id');
    }

    public function deleter(){
        return $this->belongsTo('App\Models\User', 'deleted_by', 'id');
    }
    
    public function updater(){
        return $this->belongsTo('App\Models\User', 'updated_by', 'id');
    }
}
