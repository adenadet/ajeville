...
class RequestController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $existing = Domiciliary::where('patient_id', '=', $request->input('patient_id'))->where('status', '=', 1)->count();
        ...
        return response()->json([
            'domiciliaries' => Domiciliary::where('status', '>=', 1)->where('status', '<=', 5)->orderBy('start_date', 'DESC')->with(['assessor.user', 'assessment', 'patient.nationality', 'tasks'])->paginate(2), 
            'nations' => Country::orderBy('name', 'ASC')->get(), 
            'patients' => Patient::select('id', 'last_name', 'first_name', 'middle_name')->orderBy('last_name', 'ASC')->get(),
            'employees' => Employee::orderBy('unique_id', 'ASC')->with(['user', 'qualification'])->get(), 
        ]);
