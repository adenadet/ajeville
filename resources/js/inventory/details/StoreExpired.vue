...
<div class="card">
    <div class="card-header bg-dark">
        <h3 class="card-title">Expire Items</h3>
    </div>
    <div class="card-body table-responsive p-0">
        <table class="table table-hover table-striped text-nowrap">
            ...
            <tbody>
                <tr>
                    <td colspan="5">No Item has expired</td>
                </tr>
            </tbody>
        </table>
    </div>
...
export default {
    ...
    methods:{
        ...
        getInitials(page=1){
            this.loading = true;
            axios.get('/api/inventory/store_items?specific=se&page='+page)
            .then(response =>{
                this.refreshPage(response);
                this.loading = false;
                this.$toast.fire({
                    icon: 'success',
                    title: 'Store Items loaded successfully',
                });
            })
            .catch(()=>{
                this.loading = false;
                this.$toast.fire({
                    icon: 'error',
                    title: 'Store Items not loaded successfully',
                })
            });
        },
