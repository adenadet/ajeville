...
<form id="register_form">    
    ...
    <button @submit.prevent="updateNextofKin" type="submit" name="submit" class="submit btn btn-success">Submit </button>
</form>
</template>
<script>
export default {
    ...
    methods:{
        createNextofKin(){
            ...
            .then(response =>{
                ...
                Swal.fire({icon: 'success', title: 'The Next of Kin details has been created', showConfirmButton: false, timer: 1500});
            })
            .catch(()=>{
                Swal.fire({
                    icon: 'error',
        ...
        updateNextofKin(){
            this.$Progress.start();
            this.nokForm.post('/api/emr/hims/nok/'+this.nokForm.patient_id)
            .then(response =>{
                this.$Progress.finish();
                Swal.fire({icon: 'success', title: 'The Next of Kin details has been updated', showConfirmButton: false, timer: 1500});
            })
            .catch(()=>{
    ...
    mounted() {
        ...
        Fire.$on('NextOfKinFill', nok =>{this.editMode = true; if (nok != null){this.nokForm.fill(update);}
        });
    },
    props:{
        'nok': Object,
