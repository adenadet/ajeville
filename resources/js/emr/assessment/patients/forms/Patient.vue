    ...
    <div class="row">
        <div class="col-sm-2">
            <div class="form-group">
                <label>Title*</label>
                <input type="text" class="form-control" id="title" name="title" placeholder="Title *" required v-model="ApplicantData.title" :class="{'is-invalid' : ApplicantData.errors.has('title') }" />
            </div>
        </div>
        <div class="col-sm-3">
            <div class="form-group">
...
export default {
    ...
    methods:{
        createApplicant(){
            this.$Progress.start();
            this.ApplicantData.post('/api/emr/hims/patients')
            .then(response =>{
        ...
        updateApplicantData(){
            console.log("Tested");
            this.$Progress.start();
            this.ApplicantData.put('/api/emr/hims/patients/'+this.ApplicantData.id)
            .then(response =>{
                this.$Progress.finish();
                Fire.$emit('refreshAppointment', response);
                Swal.fire({
