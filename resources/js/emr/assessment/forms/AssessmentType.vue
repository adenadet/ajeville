...
export default {
    ...
    methods: {
        createType() {
            this.$Progress.start();
            this.AssessmentTypeForm.post('/api/emr/assessments/types')
            .then(response => {
                this.$Progress.finish();
                Fire.$emit('refreshType', response);
                Swal.fire({
                    icon: 'success',
                    title: 'The Grouped Assessment has been created',
