        ...
        <div class="modal fade" id="transactionModal">
            <div class="modal-dialog modal-lg">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <!--EMRFinanceDetailPatientTransaction :transaction.sync="transaction"/-->
            ...
            <table class="table table-striped table-hover text-nowrap">
                ...
                <tbody v-if="!(loading) && transactions != null && transactions.length != 0">
                    <tr v-for="(transaction, index) in transactions" :key="transaction.id" :class="transaction.status == 0 ? 'text-danger' : ''">
                        ...
                        <td>{{ transaction?.item?.service_type?.name || 'No Service Type'}}</td>
                        <td>{{ transaction.item_name }}</td>
                        <td>{{ currency(transaction.item_total) }}</td>
                        <td>{{ transaction.status == 1 ? 'Unpaid' : 'Paid'  }}</td>
...
export default {
    components:{
        EMRFrontOfficeFormPatientService, EMRFinanceFormDeposit, EMRFinanceDetailPatientTransaction, EMRFinanceDetailTransaction
    },
    computed:{
        patient(){
