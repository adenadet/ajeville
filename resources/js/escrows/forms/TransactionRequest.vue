<template>
    <section>
    <form @submit.prevent="createTransactionRequest()">
        <div class="row">
        ...
        <div class="row">
            <div class="col-md-12" v-if="item_type == 'purchase_order'">
                <div class="form-group">
                    ...
                    <select v-if="item == null || item.id == null" class="form-control" v-model="otherCostData.po_id">
                    ...
                    <div class="form-control">
                        ...
                        <input type="hidden" v-model="otherCostData.po_id">
                    </div>
                </div>
            </div>
            <div class="col-md-12" v-else-if="item_type == 'work_order'">
                <div class="form-group">
                    ...
                    <select v-if="item == null || item.id == null" class="form-control" v-model="otherCostData.wo_id">
                        <option value="">--Select Work Order </option>
                        <option v-for="work_order in work_orders" :value="work_order.id">{{ work_order.name }} [{{ work_order.unique_id }}]</option>
                    </select>
                    <div class="form-control">
                        {{ item.name }} [{{ item.unique_id }}]
                        <input type="hidden" v-model="otherCostData.wo_id">
                    </div>
                </div>
            </div>
        </div>
...
export default {
    ...
    watch:{
        item(){
            if (this.item_type == 'purchase_order'){this.otherCostData.po_id = this.item.id;}
            else if (this.item_type == 'work_order'){this.otherCostData.wo_id = this.item.id;}
        },
    }
}
</script>