{{-- resources/views/emails/vendor-notification.blade.php --}}
@extends('mails.layouts.nairafy')

@section('header')
    <h3>Hello {{ $seller->company->name ?? $seller->first_name }},</h3>  
    <p>We're happy to inform you that the buyer <strong>{{ $buyer->first_name }} {{$buyer->last_name}}</strong> ({{ $buyer->email }}) has made payment for your product/service.</p>  
    <p>You will receive the amount of <strong>₦{{ number_format($payment->amount, 2) }}</strong> after our processing.</p>
    <p>We'll keep you updated as the transaction progresses.</p>  
@endsection
@section('message')
