    ...
    <ul class="navbar-nav ml-auto">
        <li class="nav-item dropdown">
            <a id="dropdownSubMenu2" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle">
                @if(isset($icon))<i class="{{$icon}} mr-1"></i>
                @endif
                {{$page_title}}
            </a>
            <ul aria-labelledby="dropdownSubMenu2" class="dropdown-menu border-0 shadow">
                ...
                @endif
                @if(Auth::user()->hasRole('Coop') || Auth::user()->hasRole('Super Admin'))
                ...
                @endif
                @if(Auth::user()->hasRole('Facility') || Auth::user()->hasRole('Super Admin'))
                ...
                <li class="dropdown-submenu dropdown-hover">
                    ...
                    <ul aria-labelledby="dropdownSubMenu4" class="dropdown-menu border-0 shadow">
                        @if(Auth::user()->hasRole('Learn User') || Auth::user()->hasRole('Super Admin'))<li><a href="/learn/student/dashboard" class="dropdown-item"><i class="fas fa-chalkboard nav-icon mr-1"></i>Student</a></li>@endif
                        @if(Auth::user()->hasRole('Learn Tutor') || Auth::user()->hasRole('Super Admin'))<li><a href="/learn/tutor/dashboard" class="dropdown-item"><i class="fas fa-chalkboard-teacher mr-1 nav-icon"></i>Tutor</a></li>@endif
                        @if(Auth::user()->hasRole('Learn Admin') || Auth::user()->hasRole('Super Admin'))<li><a href="/learn/admin/dashboard" class="dropdown-item"><i class="fas fa-user-cog nav-icon mr-1"></i>Admin</a></li>@endif
                    </ul>
                </li>
                @endif
                ...
                @endif
                @if(Auth::user()->hasRole('Sales Order') || Auth::user()->hasRole('Super Admin'))
        ...
        <li class="nav-item d-none d-sm-inline-block dropdown"><header-branch :branches="{{Auth::user()->branches}}" :staff_branch="{{Auth::user()->branch}}" /></li>
        <li class="nav-item dropdown">
        ...
        <li class="nav-item dropdown">
            <a id="dropdownSubMenu1" href="#" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" class="nav-link dropdown-toggle">{{Auth::user()->first_name}} &nbsp; <img height="30px" src="{{asset('img/profile/'.Auth::user()->image)}}" class="img-circle elevation-2" alt="{{Auth::user()->first_name}} {{Auth::user()->last_name}}">
            </a>
            <ul aria-labelledby="dropdownSubMenu1" class="dropdown-menu border-0 shadow">
                ...
                <li>
                    <a href="{{ route('logout') }}" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" class="dropdown-item"><i class="fas fa-power-off mr-1"></i>Log Out</a>
